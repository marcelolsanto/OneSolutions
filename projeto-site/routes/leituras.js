...
router.get('/ultimas/:idcaminhao', function(req, res, next) {
	...
	const instrucaoSql = `select top ${limite_linhas} 
						...
						umidade, 
						dataTemp,
						FORMAT(dataTemp,'%M/%d:HH:mm:ss') as momento_grafico
						from registro
						where fkSensor = ${idcaminhao}
...
router.get('/dashboard_one/:idcaminhao', function(req, res, next) {
	console.log('Recuperando caminhões');

	//var idcaminhao = req.body.idcaminhao; // depois de .body, use o nome (name) do campo em seu formulário de login
	var idcaminhao = req.params.idcaminhao;

	let instrucaoSql = `select top 1 temperatura, umidade, FORMAT(dataTemp,'%M/%d:HH:mm:ss') as momento_grafico from registro where fkSensor = ${idcaminhao} order by idRegistro desc`;
